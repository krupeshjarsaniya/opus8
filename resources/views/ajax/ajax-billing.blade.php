    ...
    <div class="  border-top border-gray">
        <form action="#" class="row" onsubmit="return false" method="post" name="Form_Name_{{ $value->first_name }}_{{ $value->id }}"  id="Form_Name_{{ $value->first_name }}_{{ $value->id }}">
            ...
            <div class="col-lg-3 col-12 px-3  my-3">
                <div class="row">
                    <img src="{{ $value->getProfilePicAttribute() ?? asset('assets/images/avatar-img.png') }}" alt="remedy" class="col-lg-4 col-12 rounded-circle p-0">
                    <label class="col-lg-8 mt-lg-3 col-12 text-lg-left text-center mt-3"><h5 >{{ $value->first_name }}</h5></label>
                </div>
            </div>
            <div class="col-lg-3 px-3 bg-2 form-group my-3">
